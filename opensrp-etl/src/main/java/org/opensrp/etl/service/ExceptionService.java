...
public class ExceptionService implements RegisterService<ExceptionEntity> {
    ...
    @Transactional
    public ExceptionEntity generatedEntityAndSave(JSONObject doc,
            String message, String benificiaryType) {
        try {
    ...
    @Transactional
    public ExceptionEntity generatedEntityAndSave(String caseId,
            String message, String benificiaryType, String instanceId,
            String _id) {
        try {
            exceptionEntity.setCaseId(caseId);
            exceptionEntity.setBenificiaryType(benificiaryType);
            exceptionEntity.setErrorMessage(message);
            exceptionEntity.setInstanceId(instanceId);
            exceptionEntity.setDocId(_id);
            save(exceptionEntity);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @Transactional
    public ExceptionEntity generatedEntityAndSaveForAction(JSONObject doc,
            String message, String benificiaryType) throws JSONException {
        exceptionEntity.setCaseId(doc.getString("caseID"));
        exceptionEntity.setBenificiaryType(benificiaryType);
        exceptionEntity.setErrorMessage(message);
    ...
    public void save(ExceptionEntity exceptionEntity) {
        ...
        ExceptionEntity existingExceptionEntity = findByCaseId(exceptionEntity
                .getCaseId());
