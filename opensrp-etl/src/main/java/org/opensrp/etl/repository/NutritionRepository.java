...
public class NutritionRepository implements RegisterRepository<NutritionEntity> {
    ...
    public void save(NutritionEntity nutritionEntity) {
        try {
            ...
            exceptionService.generatedEntityAndSave(nutritionEntity
                    .getRelationalid(), e.fillInStackTrace().toString(),
                    "NutritionRepository", nutritionEntity.getINSTANCEID(),
                    nutritionEntity.getDoc_id());
        }
    ...
    public boolean delete(NutritionEntity nutritionEntity) {
        Query query = getSession().createQuery(
                "delete NutritionEntity where id = :ID");
    ...
    public NutritionEntity findByCaseIdAndServerVersion(String relationalId,
            Long serverVersion) {
        Criteria listMotherCr = getSession().createCriteria(
        ...
        return lisFilterMother.size() > 0 ? (NutritionEntity) lisFilterMother
                .get(0) : null;
    }
