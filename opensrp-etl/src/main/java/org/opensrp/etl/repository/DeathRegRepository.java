...
public class DeathRegRepository implements RegisterRepository<DeathRegEntity>{
	...
	public boolean delete(DeathRegEntity deathRegEntity) {
		Query query = getSession().createQuery("delete DeathRegEntity where id = :ID");
	...
	public DeathRegEntity findByCaseIdAndToday(String relationalId, Date death_today) {
		Criteria listDeathRegCr = getSession().createCriteria(DeathRegEntity.class);
		...
		return listDeathReg.size() > 0 ? (DeathRegEntity) listDeathReg.get(0) : null;
	}
}
