...
public class AdolescentRepository implements
        RegisterRepository<AdolescentEntity> {
    ...
    public void save(AdolescentEntity adolescentEntity) {
        try {
            ...
            exceptionService.generatedEntityAndSave(adolescentEntity
                    .getRelationalid(), e.fillInStackTrace().toString(),
                    "AdolescentRepository", adolescentEntity.getINSTANCEID(),
                    adolescentEntity.get_id());
        }
    ...
    public boolean delete(AdolescentEntity adolescentEntity) {
        Query query = getSession().createQuery(
                "delete AdolescentEntity where id = :ID");
    ...
    public AdolescentEntity findByCaseIdAndToday(String relationalid, Date today) {
        Criteria listAdolescentCr = getSession().createCriteria(
                AdolescentEntity.class);
        ...
        return listAdolescent.size() > 0 ? (AdolescentEntity) listAdolescent
                .get(0) : null;
    }
