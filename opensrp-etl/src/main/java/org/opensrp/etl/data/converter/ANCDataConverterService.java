...
public class ANCDataConverterService implements DataConverterService {
	...
	private static final Logger logger = Logger.getLogger(ANCDataConverterService.class);
	...
	public void convertToEntityAndSave(JSONObject doc) throws JSONException {
		...
		if (anc1VisitDoc.length() != 0) {
			ancEntity = (ANCEntity) dataConverter.convert(anc1VisitDoc, className, ancObject);
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "anc1");
			}
		}
		JSONObject anc2VisitDoc = new JSONObject(doc.getJSONObject("ANCVisit2").toString());
		
		if (anc2VisitDoc.length() != 0) {
			ancEntity = (ANCEntity) dataConverter.convert(anc2VisitDoc, className, ancObject);
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "anc2");
			}
		}
		JSONObject anc3VisitDoc = new JSONObject(doc.getJSONObject("ANCVisit3").toString());
		
		if (anc3VisitDoc.length() != 0) {
			ancEntity = (ANCEntity) dataConverter.convert(anc3VisitDoc, className, ancObject);
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "anc3");
			}
		}
		JSONObject anc4VisitDoc = new JSONObject(doc.getJSONObject("ANCVisit4").toString());
		
		if (anc4VisitDoc.length() != 0) {
			ancEntity = (ANCEntity) dataConverter.convert(anc4VisitDoc, className, ancObject);
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "anc4");
			}
		}
	}
