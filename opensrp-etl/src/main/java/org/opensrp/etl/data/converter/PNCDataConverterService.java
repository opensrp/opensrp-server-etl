...
public class PNCDataConverterService implements DataConverterService {
	...
	private static final Logger logger = Logger.getLogger(PNCDataConverterService.class);
	...
	public void convertToEntityAndSave(JSONObject doc) throws JSONException {
		...
		if (pnc1VisitDoc.length() != 0) {
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "pnc1");
			}
		}
		...
		if (pnc2VisitDoc.length() != 0) {
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "pnc2");
			}
		...
		if (pnc3VisitDoc.length() != 0) {
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "pnc3");
			}
		}
		...
		if (pnc4VisitDoc.length() != 0) {
			...
			try {
				...
				logger.error(e);
				exceptionService.generatedEntityAndSave(doc, e.fillInStackTrace().toString(), "pnc4");
			}
		}
	}
}
