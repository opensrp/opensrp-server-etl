-- FUNCTION: public.psrf_data(text[])

-- DROP FUNCTION public.psrf_data(text[]);

CREATE OR REPLACE FUNCTION public.psrf_data(
	filterarray text[])
...
ALTER FUNCTION public.psrf_data(text[])
    OWNER TO postgres;

select * from psrf_data(array['','','','','','','','','']);
