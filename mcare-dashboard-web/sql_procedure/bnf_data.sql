-- FUNCTION: public.bnf_data(text[])

-- DROP FUNCTION public.bnf_data(text[]);

CREATE OR REPLACE FUNCTION public.bnf_data(
	filterarray text[])
...
--  	END LOOP ;		
	...
	and visit_code like \'%BirthNotificationPregnancyStatusFollowUp%\'
...
ALTER FUNCTION public.encc_data(text[])
    OWNER TO postgres;

select * from bnf_data(array['','','','','','','','','']);
