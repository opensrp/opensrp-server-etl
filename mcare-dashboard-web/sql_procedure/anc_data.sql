-- FUNCTION: public.anc_data(text[])

-- DROP FUNCTION public.anc_data(text[]);

CREATE OR REPLACE FUNCTION public.anc_data(
	filterarray text[])
...
ALTER FUNCTION public.anc_data(text[])
    OWNER TO postgres;
select * from anc_data(array['','','','','','','','','']);
