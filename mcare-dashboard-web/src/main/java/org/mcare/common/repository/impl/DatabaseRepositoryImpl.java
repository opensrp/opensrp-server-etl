...
public class DatabaseRepositoryImpl implements DatabaseRepository {
	...
	public <T> List<T> findAllCompletedVisits(String className, String fieldName, String value) {
		...
		List<T> result = null;
		try {
			String sql = "select count(*) from " + className + " where " + fieldName + " = :fieldValue ";
			SQLQuery query = session.createSQLQuery(sql);
	...
	public <T> List<T> findAllANCData(String ancType) {
		Session session = sessionFactory.openSession();
		List<T> result = null;
		try {
			String sql = "SELECT (select count(*) as scheduled from action where visit_code = :ancType and is_action_active=true), "
			        + "(select count(*) as completed from anc WHERE ancname= :ancType), "
			        + "count(*) as expired FROM action " + "WHERE visit_code = :ancType and alert_status='expired'";
